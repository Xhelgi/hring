...
use eframe::{egui::{self, Align2, Color32, FontId, Key, Pos2, Sense}, emath::Rot2, epaint::{PathShape, PathStroke}};
...
impl MyApp {
        ...
        egui::SidePanel::left("left_panel")
            ...
            .show(ctx, |ui| {
                ...
                if response.changed() {
                    ...
                    self.to_worker.send((self.search_query.clone(), self.last_request_id)).ok();
                }
        ...
        egui::CentralPanel::default()
            ...
            .show(ctx, |ui| {
                ...
                for (i, group) in self.groups.iter().enumerate() {
                    ...
                    self.draw_donut_segment(&painter, center, crt_rad, end_rad, is_selected, &group.bind);
                ...
                painter.text(center, Align2::CENTER_CENTER, &self.theme.center_text, FontId::monospace(self.theme.center_text_size), Color32::WHITE);

            });
    }

    /// Renders a radar-style sweep effect to visually highlight the active group.
    pub fn draw_radar(&self, painter: &egui::Painter, center: Pos2, start_rad: f32, end_rad: f32, is_selected: bool) {
        let t = &self.theme;
        if is_selected {
            painter.add(get_shape_from_degree(
                center, start_rad, end_rad, 
                1000.0, 
                t.color_radar
            ));
        }
    ...
    pub fn draw_donut_segment(&self, painter: &egui::Painter, center: Pos2, start_rad: f32, end_rad: f32, is_selected: bool, bind: &str) {
        let t = &self.theme;
        let color = if is_selected { t.color_active } else { t.color_inactive };
        let current_radius = if is_selected { t.center_radius + t.donut_thickness } else { t.center_radius };

        painter.add(get_shape_from_degree(center, start_rad, end_rad, current_radius, color));
        ...
        painter.text(
            ...
            bind, 
            FontId { size: t.bind_text_size, family: egui::FontFamily::Monospace },
            Color32::BLACK);
        
    }

    /// Calculates orbital positions and draws branching Bézier wires connecting to application nodes.
    pub fn draw_group_apps(&self, painter: &egui::Painter, center: Pos2, group: &Group, start_rad: f32, step_rad: f32, is_selected: bool) {
        let t = &self.theme;
        let apps_count = group.apps.len() as f32;
        if apps_count == 0.0 { return; }
        ...
        for (j, app) in group.apps.iter().enumerate() {
            ...
            let app_pos = center + egui::vec2(f32::cos(crt_app_rad), -f32::sin(crt_app_rad)) * orbit_radius;

            // Resolve colors based on current interactivity state.
            let circle_color = if is_selected { t.color_app_active } else { t.color_app_inactive };
            let line_color = if is_selected { t.color_line_active } else { t.color_line_inactive };
            let text_color = if is_selected { t.color_text_active } else { t.color_text_inactive };
            ...
            let p0 = center + egui::vec2(f32::cos(sector_center_rad), -f32::sin(sector_center_rad)) * (orbit_radius * t.p_start_length);
            let p1 = center + egui::vec2(f32::cos(sector_center_rad), -f32::sin(sector_center_rad)) * (orbit_radius * t.p_trunk_length);
            let p2 = center + egui::vec2(f32::cos(crt_app_rad), -f32::sin(crt_app_rad)) * (orbit_radius * t.p_branch_curve);
            let p3 = app_pos;

            let line_width = if is_selected { t.line_stroke_active_width } else { t.line_stroke_inactive_width };
            ...
            let curve = egui::epaint::CubicBezierShape::from_points_stroke(
                [p0, p1, p2, p3], false, Color32::TRANSPARENT, stroke
            );
            painter.add(curve);
            
            // Render text label and node icon.
            let background_color = if is_selected { t.color_text_background_active } else { t.color_text_background_inactive };
            self.draw_app_text(painter, app_pos, crt_app_rad, &app.name, text_color, background_color);
    ...
    pub fn draw_app_text(&self, painter: &egui::Painter, app_pos: Pos2, angle_rad: f32, text: &str, text_color: Color32, background_color: Color32) {
        let t = &self.theme;
        let galley = painter.layout_no_wrap(text.to_string(), FontId::proportional(t.font_size), text_color);
        ...
        let text_angle = if is_left_side { -angle_rad + std::f32::consts::PI } else { -angle_rad };
        ...
        let corners = [local_rect.left_top(), local_rect.right_top(), local_rect.right_bottom(), local_rect.left_bottom()];
        ...
        painter.add(PathShape::convex_polygon(bg_points, background_color, PathStroke::NONE));
